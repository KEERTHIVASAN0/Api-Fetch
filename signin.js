class FormValidation{
    ...
    validateUsername(){
        if(this.formValues.username === ""){
            ...
            this.errorValues.usernameErr = "Username should not morethan 10Characters"
            this.showErrorMsg(0,this.errorValues.usernameErr)
        } else {
    ...
    alertMessage(){
        ...
        if(usernameErr === "" && emailErr === "" && phonenumberErr === "" && passwordErr === "" && confirmpasswordErr === ""){
            swal("Registration Successful","ThankYou ,","success").then(() => {
                console.log(this.formValues)
                this.removeInputs()
            })
        } else {
            swal("Enter Valid Inputs","Click ok to Continue" ,"error")
        }
    }
